...
import { DarkRooms_backend } from "../../declarations/DarkRooms_backend";
...
const RegisterLoginUser = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isLoggingIn) {
      const loginRes = await DarkRooms_backend.verifyuser(
        username,
        password
      );
      if (loginRes == "Success") {
      ...
      if (password === confirmPassword) {
        const signUpRes = await DarkRooms_backend.registerUser(
          username,
          password
        );
        if (signUpRes == true) {
